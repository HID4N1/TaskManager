...
async def main():
    ...
    async with async_playwright() as p:
        ...
        # Auth pages (before login - use a new page to avoid session)
        print("\n=== Taking screenshots of Auth pages ===")
        auth_page = await context.new_page()
        await take_screenshot(auth_page, f"{base_url}/Auth/Login", "01_auth_login.png")
        screenshots_taken.append("01_auth_login.png")
        
        await take_screenshot(auth_page, f"{base_url}/Auth/Signup", "02_auth_signup.png")
        screenshots_taken.append("02_auth_signup.png")
        await auth_page.close()
        
        # Ensure we're logged in for authenticated pages
        current_url = page.url
        if "/Project" not in current_url and "/Home" not in current_url:
        ...
        # Get a user ID for EditUser page (skip admin user)
        try:
            await page.goto(f"{base_url}/Admin/Users", wait_until="networkidle")
            await page.wait_for_timeout(1000)
            user_links = await page.query_selector_all('a[href*="/Admin/EditUser/"]')
            if user_links:
                href = await user_links[0].get_attribute('href')
                if href:
                    import re
                    match = re.search(r'/Admin/EditUser/([^"]+)', href)
                    if match:
                        user_id = match.group(1)
                        await take_screenshot(page, f"{base_url}/Admin/EditUser/{user_id}", "16_admin_edit_user.png")
                        screenshots_taken.append("16_admin_edit_user.png")
        except Exception as e:
            print(f"Could not capture EditUser page: {e}")
        
        # Access Denied page (use a page without auth)
        print("\n=== Taking screenshots of Access Denied page ===")
        try:
            denied_page = await context.new_page()
            # Try to access an admin-only page without being admin (logout first)
            await page.goto(f"{base_url}/Auth/Logout", wait_until="networkidle")
            await page.wait_for_timeout(1000)
            await denied_page.goto(f"{base_url}/Admin/Users", wait_until="networkidle")
            await page.wait_for_timeout(2000)
            # Check if we're on access denied page
            if "Access Denied" in await denied_page.content() or "denied" in denied_page.url.lower():
                await take_screenshot(denied_page, denied_page.url, "19_access_denied.png")
                screenshots_taken.append("19_access_denied.png")
            await denied_page.close()
            # Login again
            await login(page, base_url)
        except Exception as e:
            print(f"Could not capture AccessDenied page: {e}")
        
        # Report pages
        print("\n=== Taking screenshots of Report pages ===")
        await take_screenshot(page, f"{base_url}/Report", "17_report_index.png")
        screenshots_taken.append("17_report_index.png")
        
        await take_screenshot(page, f"{base_url}/Report/TaskByStatus", "18_report_task_by_status.png")
        screenshots_taken.append("18_report_task_by_status.png")
        
        if project_ids:
            await take_screenshot(page, f"{base_url}/Report/TaskByStatus?projectId={project_ids[0]}", 
                                "19_report_task_by_status_project.png")
            screenshots_taken.append("19_report_task_by_status_project.png")
